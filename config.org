...
#+begin_src emacs-lisp :tangle yes
;; REVIEW use-package :if keyword doesn't seem to work
(if (daemonp)
...
(use-package company
  ...
  (general-imap "TAB" (general-predicate-dispatch nil
           ...
           #'yas-expand
           (and company-candidates-length (>= company-candidates-length 1))
...
(use-feature dired
  ...
  (defun open-file-external ()
    "Open file at point externally with xdg-open."
    (interactive)
    (let ((file (dired-filename-at-point)))
      (call-process
       "xdg-open" nil 0 nil (expand-file-name file))))
...
(use-feature ob
  ...
  (add-hook 'org-tab-first-hook (defun +org-src-block-tab-completion ()
                                  (if (org-in-src-block-p t)
                                      (or (completion-at-point) t))))
