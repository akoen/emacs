...
(use-package org
  ...
  (setq org-use-fast-todo-selection t) ; allow changing to any todo state from a menu
  (setq org-enforce-todo-dependencies t) ; block setting task to DONE if there are incomplete subtasks
